...
- **Revisions button scope:** a small button next to the per-file sync button
  that opens an inline panel showing the last 5 Drive revisions (time + editor).
  This is read-only context — a "here's the edit history" affordance, not a
  version switcher. Each entry can open its `exportLinks` download as a
  fallback for "what did this look like before," but the UI copy does not claim
  to jump the user into a live version.
...
- Background message handler `GET_DOC_REVISIONS` in
  [`apps/extension/entrypoints/background.ts`](../apps/extension/entrypoints/background.ts).
- Content-script helper `getDocRevisionsViaBackground(docId)` in
  [`apps/extension/lib/adapters/messages.ts`](../apps/extension/lib/adapters/messages.ts).
- Per-file **History** button in
  [`apps/extension/entrypoints/github-buttons.content.tsx`](../apps/extension/entrypoints/github-buttons.content.tsx)
  that fetches and renders the last 5 revisions in a small inline panel.
