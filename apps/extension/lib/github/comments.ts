...
export async function updatePRComment(
  ...
  const url = `https://api.github.com/repos/${owner}/${name}/issues/comments/${commentId.toString()}`;
...
export async function createReviewCommentReply(
  ...
  if (!resp.ok) {
    throw new Error(`GitHub reply failed: ${resp.status.toString()} ${await resp.text()}`);
  }
